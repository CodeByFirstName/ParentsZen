...
import { useAuth } from "../contexts/AuthContext"; // ✅ Importer le contexte
...
export default function BabysitterProfileForm({ onComplete }) {
  ...
  const { setIsProfileComplete } = useAuth(); // ✅ Utiliser le contexte
    ...
    try {
      ...
      for (let key in formData) {
        if (key === "availability") {
          ...
          dataToSend.append(key, JSON.stringify(skillsArray));
        } else if (key === "languages") {
          dataToSend.append(key, JSON.stringify(languagesArray));
        } else {
      ...
      // ✅ Marquer le profil comme complété dans le contexte et localStorage
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8 overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            onChange={handleChange}
            placeholder="Ex : Jeux éducatifs, premiers soins..."
            className="mb-4 p-2 border rounded w-full"
            required
          />
          ...
          <button
            type="submit"
            className="bg-orange-400 hover:bg-orange-500 text-white font-bold py-3 rounded-xl w-full"
          >
